...
class Slope(PinballComponent):
    ...
    def __init__(self, pos:Vector2, variant:Variant):
        if (variant == Slope.Variant.NORMAL_LEFT or 
            variant == Slope.Variant.NORMAL_RIGHT):
        ...
        if (variant == Slope.Variant.NORMAL_LEFT or 
            variant == Slope.Variant.EDGE_RIGHT):
            #Bild spiegeln
            self.sprite = pygame.transform.flip(self.sprite, True, False)
        super().__init__(pos)
        
        if (variant == Slope.Variant.NORMAL_LEFT or 
            variant == Slope.Variant.EDGE_LEFT):
            #Schräge geht von oben links nach unten rechts
            self.slopeVector = Vector2(self.rect.bottomright) - Vector2(self.rect.topleft)
        else:
            #Schräge geht von oben rechts nach unten links
            self.slopeVector = Vector2(self.rect.bottomleft) - Vector2(self.rect.topright)

        if (variant == Slope.Variant.EDGE_LEFT or 
            variant == Slope.Variant.EDGE_RIGHT):
            #Surface löschen, da Komponente sowieso im Hintergrund
